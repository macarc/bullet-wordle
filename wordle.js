...
function test(word, guess, answer) {
  ...
  for (const i in res) {
    if (res[i].mark === 'correct' && answer[i] != '.') console.error(i, guess, word, answer, res);
    if (res[i].mark === 'wrong-place' && answer[i] != '?') console.error(i, guess, word, answer, res);
    if (res[i].mark === 'wrong' && answer[i] != '!') console.error(i, guess, word, answer, res);
  }
}
